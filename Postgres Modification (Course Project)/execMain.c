...
bool has_equality_predicate(const char* query);
void increment_full_scan_count(Oid relid, const char* column_name);
char* extract_column_name(const char *query);
...
char* extract_column_name(const char *query) {
    char temp_query[1024]; // Temporary string to hold the query (to avoid modifying original)
    char column_name[100]; // Buffer to store the extracted column name
    int i = 0;
    char *where_pos;
    char *condition_start;
    
    char *result = (char *)malloc(1024 * sizeof(char));
    
    // Copy the query to temp_query for manipulation
    strcpy(temp_query, query);

    // Convert the query to lowercase to make it case-insensitive
    for (i = 0; temp_query[i]; i++) {
        temp_query[i] = tolower(temp_query[i]);
    }

    // Locate "where" keyword
    where_pos = strstr(temp_query, "where");
    if (where_pos == NULL) {
        free(result);
        return NULL;
    }

    // Move past "where" and any spaces to find the column name
    condition_start = where_pos + 5; // Skip "where"
    while (*condition_start && isspace(*condition_start)) {
        condition_start++;
    }

    // Extract the column name before the '=' symbol
    for (i = 0; condition_start[i] && condition_start[i] != '='; i++) {
        column_name[i] = condition_start[i];
    }
    column_name[i] = '\0'; // Null-terminate the column name string

    // Clean up column_name (strip spaces)
    ...
    strcpy(result, column_name);
    
    return result;
}


/* Function to detect equality predicates in the WHERE clause */
bool has_equality_predicate(const char *query) {
    
    char *where_pos = strstr(query, "=");
...
void increment_full_scan_count(Oid relid, const char *column_name) 
{
    ...
    FILE *file = fopen("/home/ubuntu-decs/Desktop/CS-SEM1/scanstats.txt", "a");
    char* table_name = get_rel_name(relid);
    
    if (!file) {
        file = fopen("/home/ubuntu-decs/Desktop/CS-SEM1/scanstats.txt", "w");
        if (!file) {
            elog(ERROR, "Failed to open file for scan stats");
        }
    }
					 ...
					 ScanDirection direction, uint64 count, bool execute_once)
{
	...
	//Custom
	FILE *file = fopen("/home/ubuntu-decs/Desktop/CS-SEM1/post.txt", "a");
	SeqScanState *node = castNode(SeqScanState, queryDesc->planstate);
	Oid relid;
	//Custom
	...
	//Custom
	relid = node->ss.ss_currentRelation->rd_id;
	if (has_equality_predicate(queryDesc->sourceText)) 
    	{
       
        	char *column_name = extract_column_name(queryDesc->sourceText);
	
        	if (column_name) 
        	{
        		fprintf(file, "Increasing scan count for: %s\n", column_name);
            		
            		increment_full_scan_count(relid, column_name);
        	}
    	}
    	fclose(file);
    	//Custom
